...
//#include <framework/useful_php.hpp> // for markup()
...
bool object2d::flip_x() { return (_placement) ? _placement->flip.get_x() : false; }
bool object2d::flip_y() { return (_placement) ? _placement->flip.get_y() : false; }
...
object2d &object2d::transform_on()
{
   if (!_placement)
   {
      ...
      _placement = new AllegroFlare::Placement2D();
...
{
   if (!_placement) transform_on();
   _placement->flip.set_x(flip_x);
   _placement->flip.set_y(flip_y);
   return *this;
}
